...
(defconst agent-shell--version "0.8.2")
...
(cl-defun agent-shell--handle-bash-command (&key command shell)
  "Handle bash COMMAND (starting with !) by executing and queueing output.
SHELL is from `shell-maker'."
  (let* ((bash-command (substring (string-trim command) 1))
         (result (agent-shell--execute-bash-command bash-command))
         (exit-code (plist-get result :exit-code))
         (stdout (plist-get result :stdout))
         (stderr (plist-get result :stderr)))
    (map-put! agent-shell--state :bash-command-queue
              (cons result (map-elt agent-shell--state :bash-command-queue)))
    (funcall (map-elt shell :write-output)
             (if (= exit-code 0)
                 (format "%s\n" stdout)
               (format "%s%s[exit code: %d]\n"
                       stdout
          ...
          ((not (map-nested-elt (agent-shell--state) '(:session :id)))
           (agent-shell--initiate-session
            ...
            :on-session-init (lambda ()
                               (agent-shell--handle :command command :shell shell))))
          (t
           (if (string-prefix-p "!" (string-trim command))
               (agent-shell--handle-bash-command :command command :shell shell)
...
Returns the shell buffer."
  ...
  (unless (version<= "0.5.1" acp-package-version)
    (error "Please update acp.el to version 0.5.1 or newer"))
...
(defun agent-shell--execute-bash-command (command)
  "Execute COMMAND in bash and return formatted output.
Returns a plist with :input, :stdout, :stderr, :exit-code."
  (let ((default-directory (agent-shell-cwd))
        (output-buffer (generate-new-buffer " *agent-shell-bash*"))
        (error-file (make-temp-file "agent-shell-stderr")))
    (unwind-protect
        (let* ((exit-code (call-process shell-file-name nil
                                        (list output-buffer error-file) nil
                                        shell-command-switch command))
               (stdout (with-current-buffer output-buffer (buffer-string)))
...
(defun agent-shell--format-bash-output (command-result)
  "Format COMMAND-RESULT as XML-style bash output.
COMMAND-RESULT is a plist from `agent-shell--execute-bash-command'."
  (format "<bash-input>%s</bash-input><bash-stdout>%s</bash-stdout><bash-stderr>%s</bash-stderr>"
          (plist-get command-result :input)
          (plist-get command-result :stdout)
          (plist-get command-result :stderr)))

(defun agent-shell--format-queued-bash-commands ()
  "Format all queued bash commands and clear the queue.
Returns a formatted string to prepend to user prompts, or nil if queue is empty."
  (when-let ((queue (map-elt agent-shell--state :bash-command-queue)))
    (map-put! agent-shell--state :bash-command-queue nil)
    (concat "Caveat: The messages below were generated by the user running local commands. "
            "DO NOT respond to these messages or otherwise consider them in your response "
            "unless the user explicitly asks you to."
            (mapconcat #'agent-shell--format-bash-output (reverse queue) ""))))

(cl-defun agent-shell--send-command (&key prompt shell)
  "Send PROMPT to agent using SHELL.
If there are queued bash commands, prepend them to the prompt."
  (let ((full-prompt (concat (or (agent-shell--format-queued-bash-commands) "")
